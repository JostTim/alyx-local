...
class ProbeInsertionInline(BaseAdmin):
    ...
    def _datasets(self, obj):
        ...
        for dset in obj.datasets.all().order_by("collection", "name"):
            url = reverse(
                "admin:%s_%s_change" % (dset._meta.app_label, dset._meta.model_name),
                args=[dset.id],
            )
            html += format_html(
                '<a href="{url}" ">./{}/{}</a><br></br>',
    ...
    def _subject(self, obj):
        ...
        url = reverse(
            "admin:%s_%s_change"
            % (
                obj.session.subject._meta.app_label,
...
class TrajectoryEstimateAdmin(BaseAdmin):
    ...
    def _probe_insertion(self, obj):
        ...
        url = reverse(
            "admin:%s_%s_change"
            % (
                obj.probe_insertion._meta.app_label,
...
class BrainRegionsAdmin(MPTTModelAdmin):
    ...
    def _parent(self, obj):
        ...
        return format_html(
            "<a href='{url}'>{} ({})</a>",
            str(obj.parent),
            str(obj.parent.id if obj.parent else ""),
            url=url,
        )
