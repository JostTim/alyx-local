...
class LabAdmin(BaseAdmin):
    form = LabForm
    generics = ["name", "institution", "address", "timezone", "reference_weight_pct", "zscore_weight_pct"]
    list_display = [
        "name",
        "institution",
        "address",
    ...
    fields = generics + list_select_related + ["cage_cleaning_frequency_days", "light_cycle", "repositories"]
...
class HousingAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if lab:
            ...
            self.fields["cage_cleaning_frequency_days"].initial = lab[0].cage_cleaning_frequency_days

    class Meta:
        model = Housing
        fields = ["cage_type", "cage_name", "enrichment", "light_cycle", "food", "cage_cleaning_frequency_days"]
...
class HousingAdmin(BaseAdmin):
    ...
    def get_queryset(self, request):
        qs = Housing.objects.annotate(
            nsubs=models.Count("housing_subjects", filter=Q(housing_subjects__end_datetime__isnull=True), distinct=True)
        )
        qs = qs.annotate(start_datetime=models.Min("housing_subjects__start_datetime"))
        qs = qs.annotate(
            end_datetime=models.Case(
                models.When(nsubs=0, then=models.Max("housing_subjects__end_datetime")),
    ...
    def subjects_l(self, obj):
        out = format_html_join(
            ", ", '<a href="{}">{}</a>', ((get_admin_url(sub), sub) for sub in obj.subjects_current())
        )
    ...
    def subjects_old(self, obj):
        ...
        out = format_html_join(", ", '<a href="{}">{}</a>', ((get_admin_url(sub), sub) for sub in subs))
        return format_html(out)
