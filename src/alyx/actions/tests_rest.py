...
class APIActionsTests(BaseTests):
    ...
    def test_list_water_administration_1(self):
        ...
        self.assertTrue(set(("date_time", "url", "subject", "user", "water_administered", "water_type")) <= set(d))
    ...
    def test_list_water_administration_filter(self):
        ...
        d = self.ar(response)[0]
        self.assertTrue(
            set(("date_time", "url", "subject", "user", "water_administered", "water_type", "adlib")) <= set(d)
        )
    ...
    def test_water_requirement(self):
        ...
        self.post(
            reverse("water-administration-create"), {"subject": self.subject.nickname, "water_administered": 1.23}
        )
        self.post(reverse("weighing-create"), {"subject": self.subject.nickname, "weight": 12.3})
        ...
        response = self.client.get(
            url + "?start_date=%s&end_date=%s" % (start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"))
        )
        self.ar(response)
        d = response.data
        self.assertEqual(d["subject"], self.subject.nickname)
    ...
    def test_extended_qc_filters(self):
        ...
        for i, ext_qc in enumerate(extended_qc):
            r = self.patch(reverse("session-detail", args=[ses[i].pk]), data={"extended_qc": ext_qc})
            data = self.ar(r)
            self.assertEqual(data["extended_qc"], ext_qc)

        def check_filt(filt, qs):
            d = self.ar(self.client.get(reverse("session-list") + filt))
            uuids = Session.objects.filter(pk__in=[dd["url"][-36:] for dd in d]).values_list("pk", flat=True)
            self.assertTrue(set(qs.values_list("pk", flat=True)) == set(uuids))

        check_filt("?extended_qc=tutu_bool,True", Session.objects.filter(extended_qc__tutu_bool=True))
        check_filt("?extended_qc=tutu_bool,False", Session.objects.filter(extended_qc__tutu_bool=False))
        check_filt("?extended_qc=tata_pct__gte,0.5", Session.objects.filter(extended_qc__tata_pct__gte=0.5))

        check_filt(
            "?extended_qc=tata_pct__lt,0.5,tutu_bool,True",
            Session.objects.filter(extended_qc__tata_pct__lt=0.5, extended_qc__tutu_bool=True),
        )
    ...
    def test_sessions(self):
        ...
        ses = Session.objects.get(
            subject=self.subject, users=self.superuser, lab__name="superlab", start_time__date="2018-07-09"
        )
        ...
        Note.objects.create(user=self.superuser, text="gnagnagna", content_type=ct, object_id=s1["url"][-36:])
    ...
    def test_list_retrieve_water_restrictions(self):
        ...
        response = self.client.get(url)
        d = self.ar(response)[0]
        self.assertTrue(set(d.keys()) >= set(["reference_weight", "water_type", "subject", "start_time", "end_time"]))
    ...
    def test_list_retrieve_lab_locations(self):
        ...
        # test patch
        url = reverse("location-detail", args=[l[0]["name"]])
        json_dict = {"string": "look at me! I'm a Json field", "integer": 15, "list": ["tutu", 5]}
        p = self.ar(self.patch(url, data={"json": json_dict}))
        self.assertEqual(p["json"], json_dict)

    def test_custom_django_filters(self):
        d = self.ar(self.client.get(reverse("session-list") + "?django=start_time__date__lt,2017-06-05"))
        ...
        resp = self.client.get(reverse("session-list") + "?django=~start_time__date__lt,2017-06-05")
        self.ar(resp)
