...
class WaterControlTests(TestCase):
    ...
    def setUp(self):
        ...
        for n, w in enumerate(np.linspace(25, 20, 50)):
            ...
            WaterAdministration.objects.create(water_administered=0.98, subject=self.sub, date_time=date_w)
        ...
        self.wr = WaterRestriction.objects.create(subject=self.sub, start_time=start_wr, water_type=water_type)
        # from now on new water administrations should have water_type as default
        wa = WaterAdministration.objects.create(
            water_administered=1.02, subject=self.sub, date_time=datetime.datetime.now()
        )
...
class NotificationTests(TestCase):
    def setUp(self):
        ...
        self.subject = Subject.objects.create(
            nickname="test", birth_date=to_date("2018-01-01"), lab=self.lab, responsible_user=self.user1
        )
        Weighing.objects.create(subject=self.subject, weight=10, date_time=timezone.datetime(2018, 6, 1, 12, 0, 0))
    ...
    def test_notif_weighing_0(self):
        ...
        Weighing.objects.create(subject=self.subject, weight=9, date_time=timezone.datetime(2018, 6, 9, 8, 0, 0))
        # No notification created here.
        self.assertTrue(len(Notification.objects.all()) == n)

    def test_notif_weighing_1(self):
        Weighing.objects.create(subject=self.subject, weight=7, date_time=timezone.datetime(2018, 6, 9, 12, 0, 0))
    ...
    def test_notif_weighing_2(self):
        Weighing.objects.create(subject=self.subject, weight=8.6, date_time=timezone.datetime(2018, 6, 9, 16, 0, 0))
        notif = Notification.objects.last()
    ...
    def test_notif_rule_1(self):
        nt = "mouse_water"
        nr = NotificationRule.objects.create(user=self.user1, notification_type=nt)
