...
from alyx.data import transfers
from alyx.data.models import Dataset, DatasetType, DataRepository, FileRecord
...
def _create_missing_file_records(dry_run=False):
    ...
    for s in (
        Session.objects.select_related("lab")
        .prefetch_related(
        ...
        for d in s.data_dataset_session_related.all():
            ...
            for dr in repos_to_create:
                ...
                if not dry_run:
                    fr = FileRecord.objects.create(dataset=fr.dataset, relative_path=fr.relative_path)
                    print("Created %s" % fr)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if action == "removelocal":
            ...
            if lab is None:
                self.stdout.write(
                    self.style.ERROR("Lab name should be specified to delete " "files on local server. Exiting now.")
                )
                return
            if before is None:
                self.stdout.write(
                    self.style.ERROR(
                        "Date beforeshould be specified: use the " "--before=yyyy-mm-dd flag. Exiting now."
                    )
                )
                return
            dtypes = [
        ...
        if action == "migrate":
            ...
            for d in FileRecord.objects.all().select_related("dataset"):
                ...
                if d.dataset.dataset_type_id != dt.pk:
                    print(
                        "Different dataset type for %s : old %s new %s"
