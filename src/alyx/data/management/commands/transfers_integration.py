from alyx.misc.models import Lab, LabLocation, LabMember
from alyx.data.models import DataRepository, DataRepositoryType, FileRecord, Dataset
from alyx.data.transfers import bulk_sync, _bulk_transfer, globus_delete_local_datasets, globus_delete_datasets
...
# from ..base.settings_lab import GLOBUS_CLIENT_ID
...
def get_local_globus_path():
    ...
    message = (
        "Please make a config-paths file in ./globus/lta that contains the "
...
class TestTransfers(object):
    ...
    def setUp(self) -> None:
        ...
        self.flatiron_endpoint_id = (
            DataRepository.objects.filter(globus_is_personal=False, name__icontains="flatiron")
            .first()
            .globus_endpoint_id
        )
        ...
        lab_location, _ = LabLocation.objects.get_or_create(name=f"{self.lab_name}_location", lab=self.lab)
        ...
        self.flatiron_data_repo, _ = DataRepository.objects.get_or_create(
            name=name,
            repository_type=repo_type,
            hostname=hostname,
            data_url=data_url,
            globus_path=self.flatiron_globus_path,
        ...
        self.local_globus_path = str(Path(local_globus_path).joinpath(self.lab_name, "Subjects")) + "/"
        self.local_data_repo, _ = DataRepository.objects.get_or_create(
            name=name,
            repository_type=repo_type,
        ...
        self.aws_data_repo, _ = DataRepository.objects.get_or_create(
            name=name,
            repository_type=repo_type,
            hostname=hostname,
            data_url=data_url,
            globus_path=aws_globus_path,
            globus_endpoint_id=self.flatiron_endpoint_id,
            globus_is_personal=globus_is_personal,
        )
        ...
        subject, _ = Subject.objects.get_or_create(nickname=nickname, lab=self.lab, responsible_user=self.user)

        # And a session associated with the subject
        self.session, _ = Session.objects.get_or_create(
            subject=subject, location=lab_location, lab=self.lab, start_time="2021-03-03", number=1
        )

        self.session_path = str(
            Path(self.session.subject.nickname, str(self.session.start_time), "00" + str(self.session.number))
        )
    ...
    def test_transfers(self):
        ...
        data = {
            ...
            "filenames": [d.relative_to(local_path + self.session_path).as_posix() for d in dsets_list],
            "server_only": False,
            "filesizes": [d.stat().st_size for d in dsets_list],
        }

        r = self.client.post(reverse("register-file"), data, SERVER_NAME=SERVER_NAME, content_type="application/json")

        exp_files = []
        exp_files_uuid = []
        for data in r.data:
            exp_files.append(str(Path(self.session_path).joinpath(collection, revision or "", data["name"])))
            exp_files_uuid.append(
                str(
                    Path(self.session_path).joinpath(
                        collection,
                        (revision or ""),
                        Path(data["name"]).stem + "." + str(data["id"]) + Path(data["name"]).suffix,
                    )
                )
            )
        ...
        self.assert_bulk_sync(exp_files, lab_name=self.lab_name, revision_name=None, iteration=1)
        ...
        bulk_sync(dry_run=False, lab=self.lab_name, gc=self.gtc)
        self.assert_bulk_sync(exp_files, lab_name=self.lab_name, revision_name=None, iteration=2)

        """
        TEST DELETING DATA
        """
        # Test deletion of local files globus_delete_local_datasets
        dsets_to_del = Dataset.objects.filter(session__lab__name=self.lab_name, name="spikes.amps.npy")
        ...
        ls_local = self.gtc.operation_ls(self.local_endpoint_id, path=str(Path(exp_files[0]).parent))
        ls_files = [ls["name"] for ls in ls_local["DATA"]]
        assert not Path(exp_files[0]).name in ls_files

        dsets_to_del = Dataset.objects.filter(session__lab__name=self.lab_name, name="spikes.times.npy")
        ...
        dsets_to_del = Dataset.objects.filter(session__lab__name=self.lab_name, name__icontains="clusters")
        frs = FileRecord.objects.filter(dataset__in=dsets_to_del)
        frs_local = frs.filter(data_repository__globus_is_personal=True).order_by("dataset__name")
        frs_server = frs.filter(
            data_repository__globus_is_personal=False, data_repository__name__icontains="flatiron"
        ...
        time.sleep(5)
        ls_flatiron = self.gtc.operation_ls(self.flatiron_endpoint_id, path=str(Path(exp_files_server[0]).parent))
    ...
    def assert_bulk_sync(expected_files, lab_name="testlab", revision_name=None, iteration=1):
        ...
        if not revision_name:
            frs_local = FileRecord.objects.filter(
                data_repository__lab__name=lab_name,
                data_repository__globus_is_personal=True,
                dataset__revision__isnull=True,
            ).order_by("dataset__name")
            frs_flatiron = FileRecord.objects.filter(
                data_repository__lab__name=lab_name,
        ...
        assert all([fr.relative_path == expected_files[iF] for iF, fr in enumerate(frs_flatiron)])
    ...
    def assert_bulk_transfer_dry(tm, expected_files, expected_files_uuid, lab_name="testlab"):
        ...
        :return:
        """
        data_repo_local = DataRepository.objects.filter(lab__name=lab_name, globus_is_personal=True).first()
        data_repo_flatiron = DataRepository.objects.filter(
            lab__name=lab_name, name__icontains="flatiron", globus_is_personal=False
        ).first()
    ...
    def tearDown(self) -> None:
        ...
        flatiron_delete = globus.DeleteData(self.gtc, str(self.flatiron_endpoint_id), recursive=True)
