...
class NoteSerializer(serializers.ModelSerializer):
    ...
    content_type = BaseSerializerContentTypeField(
        read_only=False,
        slug_field="model",
        queryset=ContentType.objects.filter(app_label__in=["actions", "data", "experiments", "jobs", "subjects"]),
    )
...
class LabSerializer(serializers.HyperlinkedModelSerializer):
    ...
    repositories = serializers.SlugRelatedField(
        read_only=False, slug_field="name", queryset=DataRepository.objects.all(), many=True, required=False
    )
