...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        subjects = Subject.objects.filter(reduced_date__isnull=False, reduced=False).order_by("nickname")
        n = len(subjects)
        ...
        subjects = Subject.objects.filter(cull__isnull=False, to_be_culled=True).order_by("nickname")
        n = len(subjects)
