...
def _repr_log_entry(l):
    ...
    if changed and changed[0].get("changed", {}):
        changed = "(%s)" % (", ".join(changed[0].get("changed", {}).get("fields", {})))
    else:
        changed = ""
    s = "%02d:%02d - %s <%s> %s" % (
        l.action_time.hour,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("-U", "--users", nargs="*", help="Usernames")
        parser.add_argument("names", nargs="*", help="List of reports to make")
        parser.add_argument("--list", action="store_true", default=False, help="List of available reports")
        parser.add_argument(
            "--no-email", action="store_true", default=False, help="Show report without sending an email"
        )
    ...
    def _generate_email(self, *args, **options):
        ...
        users = (
            get_user_model().objects.filter(username__in=users).order_by("username")
            if users
        ...
        for w in wr:
            ...
            s = """
                * {sn} since {sd}.
                ...
                """.format(
                sn=sn, sd=sd, wy=wy, wye=wye, wyep=wyep, n=n, way=way, waym=waym, waye=waye, wr=wr
            )  # noqa
            text += dedent(s)
        return text
    ...
    def make_mouse_weight(self):
        ...
        for subject_id in subject_ids:
            ...
            if wc.weight_status() > 0:
                text += (
                    "* {subject} ({user} <{email}>) weighed {weight:.1f}g "
    ...
    def make_surgery(self, user):
        ...
        surgery_done = set(
            [
                surgery.subject.nickname
                for surgery in Surgery.objects.filter(subject__responsible_user=user, subject__cull__isnull=True)
            ]
        )
        subjects_user = set(
            [subject.nickname for subject in Subject.objects.filter(responsible_user=user, cull__isnull=True)]
        )
    ...
    def make_training(self, user):
        ...
        for w in wr:
            ...
            dates = sorted(set([_[0] for _ in sessions.order_by("start_time").values_list("start_time__date")]))
            if len(dates) < 5:
                text += "* %s (%s) was trained %d days: %s\n" % (
                    w.subject.nickname,
                    w.subject.responsible_user.username,
                    len(dates),
    ...
    def make_todo(self):
        ...
        tbc = Subject.objects.filter(cull__isnull=True, to_be_culled=True).order_by("nickname")
        tbr = Subject.objects.filter(cull__isnull=False, reduced=False).order_by("nickname")
