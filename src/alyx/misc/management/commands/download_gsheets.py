...
from ....base.utils import Bunch, flatten
            ...
            yield OrderedDict(
                (
                    ...
                    (
                        "fields",
                        (
                            {k: _transform(k, v) for k, v in item.items() if k != "pk"}
...
class GoogleSheetImporter(object):
    ...
    def _download_tables(self):
        ...
        self.current_lines_table = sheet_to_table(self._line_doc.worksheet("Current lines in the " "unit"))
        ...
        self.breeding_pairs_table = sheet_to_table(
            self._line_doc.worksheet("Breeding Pairs"), header_line=2, first_line=3
        )
    ...
    def _get_line_sequences(self, line_table):
        if not line_table:
            return
        return sorted(set(c[8:].strip() for c in line_table[0].keys() if c.startswith("Genotype") and c[8:].strip()))
    ...
    def _get_subjects_in_line(self, line, table):
        ...
        for i, row in enumerate(table):
            ...
            if "nickname" not in fields:
                ...
                break
            fields["lamis_cage"] = (
                int(re.sub("[^0-9]", "", row["LAMIS Cage number"])) if row["LAMIS Cage number"] else None
            )
            ...
            if bp_index:
                try:
                    ...
                    warn("BP # is not an int: %s for subject %s.", bp_index, fields["nickname"])
    ...
    def _get_litters(self, subjects):
        ...
        litters_set = sorted(set([subject.litter for subject in subjects.values()]), key=itemgetter(1))
        litters = {}
        litter_map = {}
    ...
    def _get_surgeries(self, table):
        ...
        for row in table:
            ...
            if old_name not in self.subjects:
                warn(
                    (
                        "Subject %s doesn't exist in the transgenic spreadsheet. "
            ...
            if line:
                ...
                warn("Line %s does not exist for subject %s in procedure log." % (row["Line"], new_name))
            ...
            subject["json"] = {"history": {"nickname": [{"date_time": date_time, "value": old_name}]}}

            # Add the surgery.
            surgery = Bunch()
            surgery["users"] = [
                [get_username(initials.strip())] for initials in re.split(",|/", row["Surgery Performed By"])
            ]
    ...
    def _get_breeding_pairs(self, table):
        ...
        for i, row in enumerate(table):
            ...
            try:
                ...
                warn("BP # %s is not an integer in line %s in breeding pairs sheet." % (index, line))
                continue
            ...
            if line_obj:
                ...
                warn("Line %s referenced in BP %s doesn't exist." % (line, bp["name"]))

            for field, name_col, dob_col, sex in [
                ("father", "Father name", "Father DOB", "M"),
                ...
                if "sex" in parent and parent["sex"] != sex:
                    warn("Sex mismatch for %s between line sheet %s " "and breeding pair sheet." % (name, line))
                parent["sex"] = sex
        ...
        for n, table in self.water_tables.items():
            ...
            for row in table:
                ...
                try:
                    ...
                    warn("One of the numbers in water admin sheet for %s is not " "a float." % n)
                    continue
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        make_fixture("actions.wateradministration", importer.administrations, path="13-water-administrations")
