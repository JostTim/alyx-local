...
def display(title, query, start_date=None, end_date=None):
    ...
    with open(path, "w") as f:
        with redirect_stdout(f):
            ...
            print("\t".join(("#   ", "user       ", "prot.", "born     ", "genotyped", "died     ", "EM", "nickname")))
...
class Command(BaseCommand):
    help = "Display information required by Home Office about number of animals used " "for research."
    ...
    def handle(self, *args, **options):
        ...
        display("Killed with protocol 4, %s - %s", s.filter(killed_4(start_date, end_date)), start_date, end_date)

        display("Transgenic killed and genotyped %s - %s", tkg, start_date, end_date)
        display(
            "Transgenic killed and genotyped (negative) %s - %s",
            [s for s in tkg if s.is_negative()],
            start_date,
            end_date,
        )
