...
class HousingTests(TestCase):
    ...
    def setUp(self):
        ...
        for sub in subs1:
            HousingSubject.objects.create(
                subject=sub, housing=self.hou1, start_datetime=datetime.now() - timedelta(seconds=3600)
            )
        self.hou2 = Housing.objects.create(cage_name="housing_2")
        subs2 = Subject.objects.filter(cull__isnull=True)[1:]
        for sub in subs2:
            HousingSubject.objects.create(
                subject=sub, housing=self.hou2, start_datetime=datetime.now() - timedelta(seconds=3600)
            )
    ...
    def test_housing_subjects_current(self):
        ...
        sub2 = Subject.objects.filter(
            pk__in=self.hou2.subjects_current(datetime=hs.start_datetime + timedelta(seconds=600))
        )
    ...
    def test_move_subject(self):
        ...
        HousingSubject.objects.create(housing=self.hou1, subject=sub2, start_datetime=datetime.now())
