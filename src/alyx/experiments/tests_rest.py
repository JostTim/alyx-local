...
class APISubjectsTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.session.save()
        self.dict_insertion = {"session": str(self.session.id), "name": "probe_00", "model": "3A"}
    ...
    def test_brain_regions_rest(self):
        ...
        self.ar(self.patch(reverse("brainregion-detail", args=[parent["parent"]]), data={"description": "grandpa"}))
        parent = self.ar(self.client.get(reverse("brainregion-detail", args=[br3["parent"]])))
    ...
    def test_probe_insertion_rest(self):
        ...
        for name in probe_names:
            insertion = {"session": str(self.session.id), "name": name, "model": "3A"}
            url = reverse("probeinsertion-list")
    ...
    def test_probe_insertion_dataset_interaction(self):
        ...
        for i in range(2):
            ...
            assert set(p.datasets.all().values_list("pk", flat=True)) == set(d.values_list("pk", flat=True))
        ...
        assert set(p2.datasets.all().values_list("pk", flat=True)) == set(
            Dataset.objects.filter(collection__endswith=p2.name).values_list("pk", flat=True)
        )
        p2.delete()
    ...
    def test_create_list_delete_trajectory(self):
        ...
        insertion = {"session": str(self.session.id), "name": "probe_00", "model": "3A"}
        url = reverse("probeinsertion-list")
        ...
        tdict = {
            "probe_insertion": alyx_insertion["id"],
            "x": -4521.2,
            "y": 2415.0,
            "z": 0,
            "phi": 80,
        ...
        urlf = url + "?&probe_insertion=" + alyx_insertion["id"] + "&provenance=Micro-manipulator"
        traj = self.ar(self.client.get(urlf))
        self.assertTrue(len(traj) == 1)

        urlf = url + "?&probe_insertion=" + alyx_insertion["id"] + "&provenance=Planned"
        traj = self.ar(self.client.get(urlf))
