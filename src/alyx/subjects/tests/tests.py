...
class ModelAdminTests(TestCase, metaclass=MyTestsMeta):
    ...
    def _test_list_change(self, ma):
        ...
        logger.debug("User %s, testing list %s.", self.request.user.username, ma.model.__name__)
        self.ar(r)

        # Test the add page.
        if ma.has_add_permission(self.request):
            r = ma.add_view(self.request)
            logger.debug("User %s, testing add %s.", self.request.user.username, ma.model.__name__)
            self.ar(r)
        ...
        logger.debug("User %s, testing change %s %s.", self.request.user.username, ma.model.__name__, identifier)
        self.ar(r)
    ...
    def test_zygosities_1(self):
        ...
        m.GenotypeTest.objects.create(subject=subject, sequence=sequence, test_result=1)
    ...
    def test_zygosities_2(self):
        ...
        # Create the parents.
        father = m.Subject.objects.create(nickname="father", sex="M", line=line, lab=self.lab)
        mother = m.Subject.objects.create(nickname="mother", sex="F", line=line, lab=self.lab)
        ...
        # Create the subject.
        subject = m.Subject.objects.create(nickname="subject", line=line, litter=litter, lab=self.lab)
        z = m.Zygosity.objects.filter(subject=subject).first()
        assert z is None  # no zygosity should be assigned from parents

        # Create a rule and a genotype test ; the subject should be automatically genotyped.
        zr = m.ZygosityRule.objects.create(line=line, allele=allele, sequence0=sequence, sequence0_result=1, zygosity=0)
        m.GenotypeTest.objects.create(subject=subject, sequence=sequence, test_result=1)
...
class SubjectProtocolNumber(TestCase):
    ...
    def test_protocol_number(self):
        ...
        self.surgery = Surgery.objects.create(subject=self.sub, start_time=datetime(2019, 1, 1, 12, 0, 0))
        assert self.sub.protocol_number == "2"
        # after water restriction number goes to 3
        self.wr = WaterRestriction.objects.create(subject=self.sub, start_time=datetime(2019, 1, 1, 12, 0, 0))
...
class SubjectCullTests(TestCase):
    ...
    def setUp(self):
        ...
        self.sub2 = Subject.objects.create(nickname="loretta", lab=self.lab, birth_date="2019-01-01")
        self.CO2 = CullMethod.objects.create(name="CO2")
        self.decapitation = CullMethod.objects.create(name="decapitation")
        self.wr = WaterRestriction.objects.create(subject=self.sub1, start_time=datetime(2019, 1, 1, 12, 0, 0))
    ...
    def test_update_cull_object(self):
        ...
        self.assertNotEqual(WaterRestriction.objects.get(subject=self.sub1).end_time.strftime("%Y-%m-%d"), cull.date)
