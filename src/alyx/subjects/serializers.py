...
from .models import Allele, Line, Litter, Source, Species, Strain, Subject, Zygosity, Project
...
class ZygosityListSerializer(serializers.ModelSerializer):
    ...
    allele = serializers.SlugRelatedField(
        read_only=False, slug_field="nickname", queryset=Allele.objects.all(), required=False
    )
...
class SubjectListSerializer(_WaterRestrictionBaseSerializer):
    genotype = serializers.ListField(source="zygosity_strings", required=False)
    ...
    species = serializers.SlugRelatedField(
        read_only=False, slug_field="nickname", queryset=Species.objects.all(), allow_null=True, required=False
    )

    strain = serializers.SlugRelatedField(
        read_only=False, slug_field="name", queryset=Strain.objects.all(), allow_null=True, required=False
    )

    line = serializers.SlugRelatedField(
        read_only=False, slug_field="name", queryset=Line.objects.all(), allow_null=True, required=False
    )

    litter = serializers.SlugRelatedField(
        read_only=False, slug_field="name", queryset=Litter.objects.all(), allow_null=True, required=False
    )
    ...
    source = serializers.SlugRelatedField(
        read_only=False, slug_field="name", queryset=Source.objects.all(), allow_null=True, required=False
    )
