...
if not out:
    out = _psql("CREATE USER {DBUSER} WITH PASSWORD '{DBPASSWORD}'", DBUSER=DBUSER, DBPASSWORD=DBPASSWORD)
...
try:
    _replace_in_file("alyx/alyx/settings_template.py", "alyx/alyx/settings.py", replacements=repl)
    _replace_in_file("alyx/alyx/settings_lab_template.py", "alyx/alyx/settings_lab.py")
    # Set up the settings file.
    _replace_in_file(
        "alyx/alyx/settings_secret_template.py",
        "alyx/alyx/settings_secret.py",
        replacements=repl,
    )

    # Set up the .pgpass file to avoid typing the postgres password.
    _replace_in_file("scripts/templates/.pgpass_template", "~/.pgpass", replacements=repl, target_mode="a", chmod=0o600)

    # Set up the maintainance scripts.
    _replace_in_file("scripts/templates/load_db.sh", "scripts/load_db.sh", replacements=repl, chmod=0o755)
    _replace_in_file("scripts/templates/dump_db.sh", "scripts/dump_db.sh", replacements=repl, chmod=0o755)
